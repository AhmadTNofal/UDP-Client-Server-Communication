...
int main() {
    const char* server_ip = "192.168.1.8"; // Server's IP address
    ...
    // Create server address structure and bind to the specific IP
    sockaddr_in server_addr;
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    inet_pton(AF_INET, server_ip, &server_addr.sin_addr); // Bind to a specific IP
